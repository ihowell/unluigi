...
env_type = ['atari']
reward_shaping = [0, -0.001, -0.0025, -0.005, -0.01]#, -0.1]
augmentation = ['', 'constraint_state', 'action_history']#, 'constraint_state_noembed']
...
def oned_tasks():
    ...
    constraint = ['1d_dithering2', '1d_dithering2_dense', '1d_dithering2_hard', '1d_actuation4', '1d_actuation4_dense', '1d_actuation4_hard', '1d_dithering2_hardtrain', '1d_dithering2_hardeval']
    arg_names = [
        'env_type', 'env', 'constraint', 'reward_shaping', 'augmentation',
        'train_seed', 'eval_seed'
    ]
...
def spaceinvader_tasks():
    ...
    env_type = ['atari']
    env = ['SpaceInvaders']
    constraint = ['dangerzone', 'dangerzone_dense', 'dangerzone_hard']
    arg_names = [
        'env_type', 'env', 'constraint', 'reward_shaping', 'augmentation',
        'train_seed', 'eval_seed'
    ]
...
def create_tasks():
    tasks = oned_tasks() + twod_tasks() + breakout_tasks() + spaceinvader_tasks() + seaquest_tasks()
