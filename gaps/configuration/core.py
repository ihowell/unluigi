...
DEFAULT_PARSER = 'cfg'
PARSER = os.environ.get('GAPS_CONFIG_PARSER', DEFAULT_PARSER)
...
def _check_parser(parser_class, parser):
    if not parser_class.enabled:
        msg = ("Parser not installed yet. "
               "Please, install gaps with required parser:\n"
               "pip install gaps[{parser}]")
...
def add_config_path(path):
    ...
    _check_parser(parser_class, parser)
    if parser != PARSER:
        msg = ("Config for {added} parser added, but used {used} parser. "
               "Set up right parser via env var: "
               "export GAPS_CONFIG_PARSER={added}")
...
if 'GAPS_CONFIG_PATH' in os.environ:
    add_config_path(os.environ['GAPS_CONFIG_PATH'])
