...
#
"""
gaps.configuration provides some convenience wrappers around Python's
ConfigParser to get configuration options from config files.

The default location for configuration files is gaps.cfg (or client.cfg) in the current
working directory, then /etc/gaps/client.cfg.
...
class InterpolationMissingEnvvarError(InterpolationError):
    ...
    def __init__(self, option, section, value, envvar):
        msg = ("Config refers to a nonexisting environment variable {}. "
               "Section [{}], option {}={}").format(envvar, section, option,
                                                    value)
    ...
    def _interpolate_env(self, option, section, value):
        ...
        while value:
            ...
            try:
                ...
                raise InterpolationMissingEnvvarError(option, section, rawval,
                                                      envvar)
...
class GapsConfigParser(BaseParser, ConfigParser):
    NO_DEFAULT = object()
    enabled = True
    _instance = None
    _config_paths = [
        '/etc/gaps/client.cfg',  # Deprecated old-style global gaps config
        '/etc/gaps/gaps.cfg',
        'client.cfg',  # Deprecated old-style local gaps config
        'gaps.cfg',
    ]
    ...
    def reload(cls):
        ...
        deprecated_paths = [
            p for p in cls._config_paths
            if os.path.basename(p) == 'client.cfg' and os.path.exists(p)
        ]
        if deprecated_paths:
            warnings.warn(
                "Gaps configuration files named 'client.cfg' are deprecated if favor of 'gaps.cfg'. "
    ...
    def _get_with_default(self,
        ...
        try:
            ...
            if default is GapsConfigParser.NO_DEFAULT:
                raise
