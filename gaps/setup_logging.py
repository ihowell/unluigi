...
#
"""
This module contains helper classes for configuring logging for gapsd and
...
from gaps.configuration import get_config, GapsConfigParser
...
class BaseLogging(object):
    ...
    def _section(cls, opts):
        ...
        if isinstance(cls.config, GapsConfigParser):
            return False
        try:
...
class DaemonLogging(BaseLogging):
    ...
    def _cli(cls, opts):
        ...
        If `--logdir` -- set logging with next params:
            default Gaps's formatter,
            INFO level,
            output in logdir in `gaps-server.log` file
        """
        if opts.background:
...
class InterfaceLogging(BaseLogging):
    ...
    def _conf(cls, opts):
        ...
        logging.config.fileConfig(opts.logging_conf_file,
