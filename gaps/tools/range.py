...
import gaps
from gaps.parameter import ParameterException
from gaps.target import FileSystemTarget
from gaps.task import Register, flatten_output
...
class RangeBase(gaps.WrapperTask):
    """
    ...
    start = gaps.Parameter()
    stop = gaps.Parameter()
    reverse = gaps.BoolParameter(
        default=False,
        description=
...
class RangeDailyBase(RangeBase):
    ...
    days_forward = gaps.IntParameter(
        default=0,
        description=
...
class RangeHourlyBase(RangeBase):
    ...
    hours_forward = gaps.IntParameter(
        default=0,
        description=
    ...
    def _format_datetime(self, dt):
        return gaps.DateHourParameter().serialize(dt)
...
class RangeByMinutesBase(RangeBase):
    ...
    minutes_back = gaps.IntParameter(
        ...
        description=("extent to which contiguousness is to be assured into "
                     ...
                     "outputs flapping. Increase freely if you intend to "
                     "process old dates - worker's memory is the limit"))
    minutes_forward = gaps.IntParameter(
        default=0,
        description=
    ...
    minutes_interval = gaps.IntParameter(
        default=1,
        description=
    ...
    def _format_datetime(self, dt):
        return gaps.DateMinuteParameter().serialize(dt)
...
def _constrain_glob(glob, paths, limit=5):
    ...
    while True:
        ...
        if sum(len(s) for s in char_sets.values()) > limit:
            return [
                g.replace('[0-9]', digit_set_wildcard(char_sets[g]), 1)
                for g in current
            ]
...
def _get_per_location_glob(tasks, outputs, regexes):
    ...
    positions = [
        most_common((m.start(i), m.end(i)) for m in matches)[0]
        for i in range(1, n_groups + 1)
    ]

    glob = list(
        paths[0])  # FIXME sanity check that it's the same for all paths
...
def _get_filesystems_and_globs(datetime_to_task, datetime_to_re):
    ...
    For convenience, the task can be a gaps.WrapperTask,
    ...
    sample_datetimes = [
        datetime(y, m, d, h) for y in range(2000, 2050, 10)
        for m in range(1, 4) for d in range(5, 8) for h in range(21, 24)
    ]
...
def _list_existing(filesystem, glob, paths):
    ...
    Takes a gaps.target.FileSystem object, a str which represents a glob and
    ...
    logger.debug('%d %s listings took %f s to return %d items', len(globs),
                 filesystem.__class__.__name__,
                 time.time() - time_start, len(listing))
    return set(listing)
...
def infer_bulk_complete_from_fs(datetimes, datetime_to_task, datetime_to_re):
    ...
    (Eventually Gaps could have ranges of completion as first-class citizens.
    ...
    """
    filesystems_and_globs_by_location = _get_filesystems_and_globs(
        datetime_to_task, datetime_to_re)
    paths_by_datetime = [[o.path for o in flatten_output(datetime_to_task(d))]
                         for d in datetimes]
    listing = set()
    for (f, g), p in zip(
...
class RangeMonthly(RangeBase):
    ...
    """
    start = gaps.MonthParameter(
        default=None,
        description=
        "beginning month, inclusive. Default: None - work backward forever (requires reverse=True)"
    )
    stop = gaps.MonthParameter(
        default=None,
        description=
        "ending month, exclusive. Default: None - work forward forever")
    months_back = gaps.IntParameter(
        ...
        description=(
            ...
            "flapping. Increase freely if you intend to process old "
            "dates - worker's memory is the limit"))
    months_forward = gaps.IntParameter(
        default=0,
        description=
        "extent to which contiguousness is to be assured into future, in months from current time. "
...
class RangeDaily(RangeDailyBase):
    ...
    .. code-block:: console
        ...
        gaps --module your.module RangeDaily --of YourActualTask --start 2014-01-01
    """
    def missing_datetimes(self, finite_datetimes):
        try:
            cls_with_params = functools.partial(self.of, **self.of_params)
            complete_parameters = self.of.bulk_complete.__func__(
                cls_with_params,
                map(self.datetime_to_parameter, finite_datetimes))
            return set(finite_datetimes) - set(
                map(self.parameter_to_datetime, complete_parameters))
        except NotImplementedError:
            return infer_bulk_complete_from_fs(
                finite_datetimes, lambda d: self._instantiate_task_cls(
                    self.datetime_to_parameter(d)),
...
class RangeHourly(RangeHourlyBase):
    ...
    .. code-block:: console
        ...
        gaps --module your.module RangeHourly --of YourActualTask --start 2014-01-01T00
    """
    def missing_datetimes(self, finite_datetimes):
        try:
            ...
            complete_parameters = self.of.bulk_complete.__func__(
                cls_with_params,
                list(map(self.datetime_to_parameter, finite_datetimes)))
            return set(finite_datetimes) - set(
                map(self.parameter_to_datetime, complete_parameters))
        except NotImplementedError:
            return infer_bulk_complete_from_fs(
...
class RangeByMinutes(RangeByMinutesBase):
    ...
    .. code-block:: console
        ...
        gaps --module your.module RangeByMinutes --of YourActualTask --start 2014-01-01T0123
    """
    def missing_datetimes(self, finite_datetimes):
        try:
            cls_with_params = functools.partial(self.of, **self.of_params)
