...
# Example:
#
# PYTHONPATH=$PYTHONPATH:/path/to/your/gaps/tasks bin/deps.py \
...
import gaps.interface
# from gaps.contrib.ssh import RemoteTarget
# from gaps.contrib.postgres import PostgresTarget
# from gaps.contrib.s3 import S3Target
from gaps.target import FileSystemTarget
from gaps.task import flatten
...
class upstream(gaps.task.Config):
    '''
...
def get_task_output_description(task_output):
    ...
    if isinstance(task_output, FileSystemTarget):
...
def main():
    ...
    for task in deps:
        ...
        if isinstance(task_output, dict):
            output_descriptions = [
                get_task_output_description(output)
                for label, output in task_output.items()
            ]
        elif isinstance(task_output, collections.Iterable):
            output_descriptions = [
                get_task_output_description(output) for output in task_output
            ]
        else:
