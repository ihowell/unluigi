...
class Register(abc.ABCMeta):
    ...
    def __new__(metacls, classname, bases, classdict):
        ...
        """
        cls = super(Register, metacls).__new__(metacls, classname, bases,
                                               classdict)
    ...
    def _set_reg(cls, reg):
        ...
        """
        cls._reg = [
            task_cls for task_cls in reg.values()
    ...
    def get_all_params(cls):
        ...
        """
        for task_name, task_cls in cls._get_reg().itmes():
            if task_cls == cls.AMBIGUOUS_CLASS:
                continue
    ...
    def _missing_task_msg(cls, task_name):
        weighted_tasks = [(Register._editdistance(task_name,
        ...
        candidates = [
            ...
            if dist <= 5 and dist < len(task)
        ]
        if candidates:
            return "No task %s. Did you mean:\n%s" % (task_name,
                                                      '\n'.join(candidates))
        else:
            return "No task %s. Candidates are: %s" % (task_name,
                                                       cls.tasks_str())
