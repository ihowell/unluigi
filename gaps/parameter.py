...
#
''' Parameters are one of the core concepts of gaps.
All Parameters sit on :class:`~gaps.task.Task` classes.
...
from gaps import date_interval
from gaps import task_register
...
class Parameter(object):
    ...
    .. code:: python
        ...
        class MyTask(gaps.Task):
            foo = gaps.Parameter()

        class RequiringTask(gaps.Task):
            def requires(self):
    ...
    def __init__(self,
        ...
        :param visibility: A Parameter whose value is a :py:class:`~gaps.parameter.ParameterVisibility`.
        ...
        self.visibility = visibility if ParameterVisibility.has_value(
            visibility) else ParameterVisibility.PUBLIC

        self.description = description
        self.always_in_help = always_in_help

        if config_path is not None and ('section' not in config_path
                                        or 'name' not in config_path):
    ...
    def _value_iterator(self, task_name, param_name):
        ...
        if self._config_path:
            yield (self._get_value_from_config(
                self._config_path['section'], self._config_path['name']
            ), 'The use of the configuration [{}] {} is deprecated. Please use [{}] {}'
                   .format(self._config_path['section'],
                           self._config_path['name'], task_name, param_name))
    ...
    def _warn_on_wrong_param_type(self, param_name, param_value):
        ...
        if not isinstance(param_value, str):
            warnings.warn(
                'Parameter "{}" with value "{}" is not of type string.'.format(
    ...
    def next_in_enumeration(self, _value):
        ...
        :py:mod:`gaps.execution_summary` module for pretty printing
    ...
    def _parser_kwargs(cls, param_name, task_name=None):
        return {
            ...
            "store",
            "dest":
            cls._parser_global_dest(param_name, task_name)
            if task_name else param_name,
        }
...
class OptionalParameter(Parameter):
    ...
    def _warn_on_wrong_param_type(self, param_name, param_value):
        ...
        if not isinstance(param_value, str) and param_value is not None:
            warnings.warn(
                'OptionalParameter "{}" with value "{}" is not of type string or None.'
...
class DateParameter(_DateParameterBase):
    ...
    .. code:: python
        ...
        class MyTask(gaps.Task):
            date = gaps.DateParameter(default=datetime.date.today())
    """
...
class MonthParameter(DateParameter):
    ...
    :py:class:`~gaps.date_interval.Month`.
    """
...
class YearParameter(DateParameter):
    ...
    :py:class:`~datetime.date` (ignoring the month and day values) or :py:class:`~gaps.date_interval.Year`.
    """
...
class BoolParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyTask(gaps.Task):
            implicit_bool = gaps.BoolParameter(parsing=gaps.BoolParameter.IMPLICIT_PARSING)
            explicit_bool = gaps.BoolParameter(parsing=gaps.BoolParameter.EXPLICIT_PARSING)

    or globally by

    .. code-block:: python

        gaps.BoolParameter.parsing = gaps.BoolParameter.EXPLICIT_PARSING
    ...
    def _parser_kwargs(self, *args, **kwargs):
        parser_kwargs = super(BoolParameter,
                              self)._parser_kwargs(*args, **kwargs)
...
class DateIntervalParameter(Parameter):
    """
    A Parameter whose value is a :py:class:`~gaps.date_interval.DateInterval`.
    ...
    def parse(self, s):
        """
        Parses a :py:class:`~gaps.date_interval.DateInterval` from the input.

        see :py:mod:`gaps.date_interval`
          for details on the parsing of DateIntervals.
        """
        # TODO: can we use xml.utils.iso8601 or something similar?

        from gaps import date_interval as d
...
class TimeDeltaParameter(Parameter):
    ...
    def _parseIso8601(self, input):
        ...
        regex = "P(%s|%s(T%s)?)" % (
            field("weeks"), optional_field("days"), "".join([
                optional_field(key) for key in ["hours", "minutes", "seconds"]
            ]))
    ...
    def parse(self, input):
        ...
        if result is not None:
            ...
            raise ParameterException(
                "Invalid time delta - could not parse %s" % input)
    ...
    def serialize(self, x):
        ...
        result = "{} w {} d {} h {} m {} s".format(weeks, days, hours, minutes,
                                                   seconds)
        return result
    ...
    def _warn_on_wrong_param_type(self, param_name, param_value):
        ...
        if not isinstance(param_value, datetime.timedelta):
            warnings.warn(
                'Parameter "{}" with value "{}" is not of type timedelta.'.
                format(param_name, param_value))


class TaskParameter(Parameter):
    """
    A parameter that takes another gaps task class.

    When used programatically, the parameter should be specified
    directly with the :py:class:`gaps.task.Task` (sub) class. Like
    ``MyMetaTask(my_task_param=my_tasks.MyTask)``. On the command line,
    you specify the :py:meth:`gaps.task.Task.get_task_family`. Like

    .. code-block:: console

            $ gaps --module my_tasks MyMetaTask --my_task_param my_namespace.MyTask

    Where ``my_namespace.MyTask`` is defined in the ``my_tasks`` python module.

    When the :py:class:`gaps.task.Task` class is instantiated to an object.
    The value will always be a task class (and not a string).
    """
    def parse(self, input):
        """
        Parse a task_famly using the :class:`~gaps.task_register.Register`
        """
        return task_register.Register.get_task_cls(input)

    def serialize(self, cls):
        """
        Converts the :py:class:`gaps.task.Task` (sub) class to its family name.
        """
...
class EnumParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyTask(gaps.Task):
          my_param = gaps.EnumParameter(enum=Model)

    At the command line, use,

    .. code-block:: console

        $ gaps --module my_tasks MyTask --my-param Honda
...
class EnumListParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyTask(gaps.Task):
          my_param = gaps.EnumListParameter(enum=Model)

    At the command line, use,

    .. code-block:: console

        $ gaps --module my_tasks MyTask --my-param Honda,Volvo
    ...
    def parse(self, s):
        ...
        for i, v in enumerate(values):
            try:
                ...
                raise ValueError(
                    'Invalid enum value "{}" index {} - could not be parsed'.
                    format(v, i))
...
class DictParameter(Parameter):
    ...
    .. code-block:: console
        ...
        $ gaps --module my_tasks MyTask --tags <JSON string>

    Simple example with two tags:

    .. code-block:: console

        $ gaps --module my_tasks MyTask --tags '{"role": "web", "env": "staging"}'

    It can be used to define dynamic parameters, when you do not know the exact list of your parameters (e.g. list of
    tags, that are dynamically constructed outside gaps), or you have a complex parameter containing logically related
...
class ListParameter(Parameter):
    ...
    .. code-block:: console
        ...
        $ gaps --module my_tasks MyTask --grades <JSON string>

    Simple example with two grades:

    .. code-block:: console

        $ gaps --module my_tasks MyTask --grades '[100,70]'
    """
    def normalize(self, x):
        """
...
class TupleParameter(ListParameter):
    ...
    .. code-block:: python
        ...
        class MyTask(gaps.Task):
          book_locations = gaps.TupleParameter()
    ...
    .. code-block:: console
        ...
        $ gaps --module my_tasks MyTask --book_locations <JSON string>

    Simple example with two grades:

    .. code-block:: console

        $ gaps --module my_tasks MyTask --book_locations '((12,3),(4,15),(52,1))'
    """
    def parse(self, x):
        """
        Parse an individual value from the input.
        ...
        try:
            ...
            return tuple(
                tuple(x)
                for x in json.loads(x, object_pairs_hook=FrozenOrderedDict))
        except (ValueError, TypeError):
            return tuple(literal_eval(
                x))  # if this causes an error, let that error be raised.
...
class NumericalParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyTask(gaps.Task):
            my_param_1 = gaps.NumericalParameter(
                var_type=int, min_value=-3, max_value=7) # -3 <= my_param_1 < 7
            my_param_2 = gaps.NumericalParameter(
    ...
    .. code-block:: console
        ...
        $ gaps --module my_tasks MyTask --my-param-1 -3 --my-param-2 -2
    """
    def __init__(self,
                 left_op=operator.le,
        ...
        self._permitted_range = (
            "{var_type} in {left_endpoint}{min_value}, {max_value}{right_endpoint}"
            .format(var_type=self._var_type.__name__,
    ...
    def parse(self, s):
        ...
        if (self._left_op(self._min_value, value)
                and self._right_op(value, self._max_value)):
            return value
        else:
            raise ValueError(
...
class ChoiceParameter(Parameter):
    ...
    .. code-block:: python
        ...
        class MyTask(gaps.Task):
            my_param = gaps.ChoiceParameter(choices=[0.1, 0.2, 0.3], var_type=float)

    At the command line, use

    .. code-block:: console

        $ gaps --module my_tasks MyTask --my-param 0.1

    Consider using :class:`~gaps.EnumParameter` for a typed, structured
    ...
    def __init__(self, var_type=str, *args, **kwargs):
        ...
        assert all(type(choice) is self._var_type
                   for choice in self._choices), "Invalid type in choices"
    ...
    def normalize(self, var):
        if var in self._choices:
            ...
            raise ValueError(
                "{var} is not a valid choice from {choices}".format(
