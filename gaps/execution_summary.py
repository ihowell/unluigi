...
<https://github.com/spotify/gaps/blob/master/examples/execution_summary_example.py>`_
at the end of gaps invocations.
"""
...
class execution_summary(gaps.Config):
    summary_length = gaps.IntParameter(default=5)


class GapsStatusCode(enum.Enum):
    """
    All possible status codes for the attribute ``status`` in :class:`~gaps.execution_summary.GapsRunResult` when
    the argument ``detailed_summary=True`` in *gaps.run() / gaps.build*.
...
class GapsRunResult(object):
    ...
    Attributes:
        ...
        - status (GapsStatusCode): Gaps Status Code. See :class:`~gaps.execution_summary.GapsStatusCode` for what these codes mean.
        - worker (gaps.worker.worker): Worker object. See :class:`~gaps.worker.worker`.
    ...
    def __init__(self, worker, worker_add_run_status=True):
        ...
        self.summary_text = _summary_wrap(_summary_format(
            summary_dict, worker))
    ...
    def __str__(self):
        return "GapsRunResult with status {0}".format(self.status)

    def __repr__(self):
        return "GapsRunResult(status={0!r},worker={1!r},scheduling_succeeded={2!r})".format(
            self.status, self.worker, self.scheduling_succeeded)
...
def _partition_tasks(worker):
    ...
    pending_tasks = {
        task
        for (task, status, ext) in task_history if status == 'PENDING'
    }
    set_tasks = {}
    set_tasks["completed"] = {
        task
    ...
    set_tasks["already_done"] = {
        task
        for (task, status, ext) in task_history if status == 'DONE'
        and task not in pending_tasks and task not in set_tasks["completed"]
    }
    set_tasks["ever_failed"] = {
        task
        for (task, status, ext) in task_history if status == 'FAILED'
    }
    set_tasks["failed"] = set_tasks["ever_failed"] - set_tasks["completed"]
    set_tasks["scheduling_error"] = {
        task
        for (task, status, ext) in task_history if status == 'UNKNOWN'
    }
    set_tasks["still_pending_ext"] = {
        task
        for (task, status, ext) in task_history
        if status == 'PENDING' and task not in set_tasks["ever_failed"]
        and task not in set_tasks["completed"] and not ext
    }
    set_tasks["still_pending_not_ext"] = {
        task
        for (task, status, ext) in task_history
        if status == 'PENDING' and task not in set_tasks["ever_failed"]
        and task not in set_tasks["completed"] and ext
    }
...
def _get_str(task_dict, extra_indent):
    ...
    for task_family in task_names:
        ...
        if len(tasks[0].get_params()) == 0:
            ...
            attributes = {
                getattr(task, tasks[0].get_params()[0][0])
                for task in tasks
            }
            ...
            if first is not None and last is not None and len(attributes) > 3:
                param_str = '{0}...{1}'.format(param_class.serialize(first),
                                               param_class.serialize(last))
            else:
                param_str = '{0}'.format(_get_str_one_parameter(tasks))
            line = prefix + '- {0} {1}({2}={3})'.format(
                len(tasks), task_family, tasks[0].get_params()[0][0],
                param_str)
        else:
            ranging = False
            ...
            if len(unique_param_keys) == 1:
                ...
                first, last = _ranging_attributes(attributes, param_class)
                if first is not None and last is not None and len(
                        attributes) > 2:
                    ranging = True
                    line = prefix + '- {0} {1}({2}'.format(
                        len(tasks), task_family,
                        _get_str_ranging_multiple_parameters(
                            first, last, tasks, unique_param))
            if not ranging:
                if len(tasks) == 1:
                    line = prefix + '- {0} {1}'.format(len(tasks), tasks[0])
                if len(tasks) == 2:
                    line = prefix + '- {0} {1} and {2}'.format(
                        len(tasks), tasks[0], tasks[1])
                if len(tasks) > 2:
                    line = prefix + '- {0} {1} ...'.format(
                        len(tasks), tasks[0])
        lines.append(line)
    return '\n'.join(lines)
...
def _get_str_ranging_multiple_parameters(first, last, tasks, unique_param):
    row = ''
    str_unique_param = '{0}...{1}'.format(unique_param[1].serialize(first),
                                          unique_param[1].serialize(last))
    for param in tasks[0].get_params():
        row += '{0}='.format(param[0])
        if param[0] == unique_param[0]:
            row += '{0}'.format(str_unique_param)
        else:
            row += '{0}'.format(param[1].serialize(getattr(tasks[0],
                                                           param[0])))
...
def _ranging_attributes(attributes, param_class):
    ...
    """
    next_attributes = {
        param_class.next_in_enumeration(attribute)
...
def _get_str_one_parameter(tasks):
    ...
    count = 0
    for task in tasks:
        if (len(row) >= 30 and count > 2
                and count != len(tasks) - 1) or len(row) > 200:
            row += '...'
            break
...
def _serialize_first_param(task):
    return task.get_params()[0][1].serialize(
        getattr(task,
                task.get_params()[0][0]))
    ...
    for status, human in _COMMENTS:
        ...
        if num_tasks:
            ...
            comments[status] = '{space}* {num_tasks} {human}:\n'.format(
                space=space, num_tasks=num_tasks, human=human)
    return comments
...
_PENDING_SUB_STATUSES = set(
    _ORDERED_STATUSES[_ORDERED_STATUSES.index("still_pending_ext"):])
_COMMENTS = {
    ...
    ("upstream_run_by_other_worker",
     'had dependencies that were being run by other worker'),
...
def _summary_format(set_tasks, worker):
    ...
    for status in _ORDERED_STATUSES:
        ...
        if status != 'still_pending':
            str_output += '{0}\n'.format(
                _get_str(group_tasks[status], status in _PENDING_SUB_STATUSES))
    ext_workers = _get_external_workers(worker)
    group_tasks_ext_workers = {}
    for ext_worker, task_dict in ext_workers.items():
        group_tasks_ext_workers[ext_worker] = _group_tasks_by_name_and_status(
            task_dict)
    if len(ext_workers) > 0:
        ...
        for ext_worker, task_dict in ext_workers.items():
            if count > 3 and count < len(ext_workers) - 1:
                str_output += "    and {0} other workers".format(
                    len(ext_workers) - count)
                break
            str_output += "    - {0} ran {1} tasks\n".format(
                ext_worker, len(task_dict))
            count += 1
        str_output += '\n'
...
def _create_one_line_summary(status_code):
    """
    Given a status_code of type GapsStatusCode which has a tuple value, returns a one line summary
    """
...
def _tasks_status(set_tasks):
    """
    Given a grouped set of tasks, returns a GapsStatusCode
    """
    if set_tasks["ever_failed"]:
        if not set_tasks["failed"]:
            return GapsStatusCode.SUCCESS_WITH_RETRY
        else:
            if set_tasks["scheduling_error"]:
                return GapsStatusCode.FAILED_AND_SCHEDULING_FAILED
            return GapsStatusCode.FAILED
    elif set_tasks["scheduling_error"]:
        return GapsStatusCode.SCHEDULING_FAILED
    elif set_tasks["not_run"]:
        return GapsStatusCode.NOT_RUN
    elif set_tasks["still_pending_ext"]:
        return GapsStatusCode.MISSING_EXT
    else:
        return GapsStatusCode.SUCCESS


def _summary_wrap(str_output):
    return textwrap.dedent("""
    ===== Gaps Execution Summary =====
