...
class InputPipeProcessWrapper(object):
    def __init__(self, command, input_pipe=None):
        ...
        self._process = command if isinstance(
            command, subprocess.Popen) else self.create_subprocess(command)
...
class OutputPipeProcessWrapper(object):
    ...
    def close(self):
        ...
        if self._process.returncode == 0:
            ...
            raise RuntimeError('Error when executing command %s' %
                               self._command)
