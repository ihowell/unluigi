...
Most gaps plumbers will find themselves in an awkward task parameter situation
at some point or another.  Consider the following "parameter explosion"
problem:

.. code-block:: python

    class TaskA(gaps.ExternalTask):
        param_a = gaps.Parameter()

        def output(self):
            return gaps.LocalTarget('/tmp/log-{t.param_a}'.format(t=self))

    class TaskB(gaps.Task):
        param_b = gaps.Parameter()
        param_a = gaps.Parameter()

        def requires(self):
            return TaskA(param_a=self.param_a)

    class TaskC(gaps.Task):
        param_c = gaps.Parameter()
        param_b = gaps.Parameter()
        param_a = gaps.Parameter()
...
**Approach 1**:  Parameters via command line or config instead of :func:`~gaps.task.Task.requires`.
...
.. code-block:: bash
    ...
    gaps --module my_tasks TaskC --param-c foo --TaskB-param-b bar --TaskA-param-a baz
...
.. code-block:: python
    ...
    class Params(gaps.Config):
        param_c = gaps.Parameter()
        param_b = gaps.Parameter()
        param_a = gaps.Parameter()

    class TaskA(Params, gaps.ExternalTask):
        def output(self):
            return gaps.LocalTarget('/tmp/log-{t.param_a}'.format(t=self))
...
idempotent and atomic nature of gaps tasks.  Unnecessary significant task
...
derived from :class:`~gaps.task.Task` should be undertaken with caution and avoided
where possible.


**Approach 3**: Use :class:`~gaps.util.inherits` and :class:`~gaps.util.requires`

The :class:`~gaps.util.inherits` class decorator in this module copies parameters (and
...
.. code-block:: python
    ...
    import gaps
    from gaps.util import inherits
...
The :class:`~gaps.util.requires` helper function can reduce this pattern even further.   It
does everything :class:`~gaps.util.inherits` does,
and also attaches a :class:`~gaps.util.requires` method
to your task (still all without pythonic inheritance).

But how does it know how to invoke the upstream task?  It uses :func:`~gaps.task.Task.clone`
behind the scenes!

.. code-block:: python

    import gaps
    from gaps.util import inherits, requires
...
class inherits(object):
    ...
    *New after Gaps 2.7.6:* multiple arguments support.

    Usage:

    .. code-block:: python

        class AnotherTask(gaps.Task):
            m = gaps.IntParameter()

        class YetAnotherTask(gaps.Task):
            n = gaps.IntParameter()

        @inherits(AnotherTask)
        class MyFirstTask(gaps.Task):
            def requires(self):
...
class requires(object):
    """
    Same as :class:`~gaps.util.inherits`, but also auto-defines the requires method.
    ...
    *New after Gaps 2.7.6:* multiple arguments support.
    ...
    def __call__(self, task_that_requires):
        task_that_requires = inherits(*self.tasks_to_require)(
            task_that_requires)
        ...
        def requires(_self):
            return _self.clone_parent() if len(
                self.tasks_to_require) == 1 else _self.clone_parents()
...
def delegates(task_that_delegates):
    ...
    if not hasattr(task_that_delegates, 'subtasks'):
        ...
        raise AttributeError('%s needs to implement the method "subtasks"' %
                             task_that_delegates)
...
def previous(task):
    ...
    for param_name, param_obj in params:
        ...
        if isinstance(param_obj, parameter.DateParameter):
            previous_date_params[
                param_name] = param_value - datetime.timedelta(days=1)
        elif isinstance(param_obj, parameter.DateSecondParameter):
            previous_date_params[
                param_name] = param_value - datetime.timedelta(seconds=1)
        elif isinstance(param_obj, parameter.DateMinuteParameter):
            previous_date_params[
                param_name] = param_value - datetime.timedelta(minutes=1)
        elif isinstance(param_obj, parameter.DateHourParameter):
            previous_date_params[
                param_name] = param_value - datetime.timedelta(hours=1)
    ...
    if len(previous_date_params) == 0:
        raise NotImplementedError(
            "No task parameter - can't determine previous task")
