...
It is a central concept of gaps and represents the state of the workflow.
...
from gaps.task_register import Register
from gaps.parameter import ParameterVisibility
from gaps.util import add_metaclass
...
def namespace(namespace=None, scope=''):
    ...
    *New since gaps 2.6.0:* ``scope`` keyword argument.
...
def auto_namespace(scope=''):
    ...
    .. code-block:: python
        ...
        gaps.auto_namespace(scope=__name__)
...
def task_id_str(task_family, params):
    ...
    return '{}_{}_{}'.format(task_family, param_summary,
                             param_hash[:TASK_ID_TRUNCATE_HASH])
...
class Task(object):
    """
    This is the base class of all gaps Tasks, the base unit of work in gaps.

    A gaps Task describes a unit or work.
    ...
    * :py:meth:`output` - the output :py:class:`Target` that this Task creates.
    ...
    Each :py:class:`~gaps.Parameter` of the Task should be declared as members:

    .. code:: python

        class MyTask(gaps.Task):
            count = gaps.IntParameter()
            second_param = gaps.Parameter()
    ...
    def event_handler(cls, event):
        ...
        def wrapped(callback):
            cls._event_callbacks.setdefault(cls,
                                            {}).setdefault(event,
                                                           set()).add(callback)
            return callback
    ...
    _visible_in_registry = True  # TODO: Consider using in gaps.util as well
    ...
    def task_family(self):
        ...
        Hopefully there will be less meta magic in gaps.
    ...
    def get_param_names(cls, include_significant=False):
        return [
            name for name, p in cls.get_params()
            if include_significant or p.significant
        ]
    ...
    def to_str_params(self, only_significant=False, only_public=False):
        ...
        params = dict(self.get_params())
        for param_name, param_value in self.param_kwargs.items():
            if (((not only_significant) or params[param_name].significant) and
                ((not only_public) or
                 params[param_name].visibility == ParameterVisibility.PUBLIC)
    ...
    def _get_param_visibilities(self):
        param_visibilities = {}
        params = dict(self.get_params())
        for param_name, _ in self.param_kwargs.items():
    ...
    def ready(self):
        """
        Returns whether a Task is ready to execute, i.e., all of its requirements
        are met.
...
v
    ...
    def no_unpicklable_properties(self):
        ...
        unpicklable_properties = tuple(
            gaps.worker.TaskProcess.forward_reporter_attributes.values())
        reserved_properties = {}
...
def externalize(taskclass_or_taskobject):
    ...
    .. code-block:: python
        ...
        @gaps.util.requires(externalize(MyTask))
        class RequiringTask(gaps.Task):
            pass
            ...
